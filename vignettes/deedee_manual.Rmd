...
As a part of the DeeDee universe, all functions as well as the Shiny App in this package require DeeDeeExperiment object (DDE) as their primary input. For more information on DeeDeeExperiment, please refer to the original package.

Beside the DeeDeeExperiment object, each < [DeeDee main function](#main-functions) takes a threshold for p-values that can be specified with the parameter `pthresh` (default = 0.05). Further, most functions have additional parameters that will be explained in the respective sections below. All functions produce colorblind-friendly output.

For our example, we use `dde_macrophage` given in the package data, which contains DEA results for four contrasts from the [macrophage](#example-dataset-used-in-this-vignette) data package (naive vs. IFNg, IFNg vs. both, naive vs. Salmonella, and Salmonella vs. both).
...
The function `deedee_scatter()` creates a scatterplot of logFC values of the genes in two input datasets. If the input DeeDeeExperiment contains more than two datasets (like our example `dde_macrophage` does),  the `select1` (default = 1) and `select2` (default = 2) parameters specify which ones will be plotted (selected by list index). `deedee_scatter()` includes a `color_by` parameter that can be set to "pval1" (default) or "pval2". The points will be colored according to the color scheme given on the right side of the output. 

```{r, echo = TRUE, warning = FALSE, message = FALSE}
deedee_scatter(data = dde_macrophage, select1 = 2, select2 = 3, color_by = "pval1", pthresh = 0.05)
```
...
```{r, echo = TRUE, warning = FALSE, message = FALSE}
deedee_heatmap(data = dde_macrophage, show_first = 25, show_gene_names = FALSE, dist = "manhattan", clust = "centroid", show_na = FALSE, pthresh = 0.05)
```
...
```{r, echo = TRUE, warning = FALSE, message = FALSE}
deedee_venn(data = dde_macrophage, mode = "both", pthresh = 0.05)
```
...
```{r, echo = TRUE, warning = FALSE, message = FALSE}
deedee_upset(data = dde_macrophage, mode = "both_colored", min_setsize = 15, pthresh = 0.05)
```
...
The function `deedee_qq()` compares the statistical distributions of two input datasets. If the input data list contains more than two datasets (like our example `dde_macrophage` does),  the `select1` (default = 1) and `select2` (default = 2) parameters specify which ones are to be used. If the resulting curve resembles a straight line with a slope of 1, the distributions are similar (perfect straight line = identical distributions). `deedee_qq()` includes a `color_by` parameter that can be set to `pval1` (default) or `pval2`. The points generating the curve will be colored according to the color key displayed on the right of the output. 

```{r, echo = TRUE, warning = FALSE, message = FALSE}
deedee_qq(data = dde_macrophage, select1 = 1, select2 = 3, color_by = "pval2", pthresh = 0.05)
```
...
```{r, echo = TRUE, warning = FALSE, message = FALSE}
deedee_qqmult(data = dde_macrophage, ref = 1, pthresh = 0.05)
```
...
```{r, echo = TRUE, warning = FALSE, message = FALSE}
deedee_cat(data = dde_macrophage, ref = 2, maxrank = 800, pthresh = 0.05)
```

\pagebreak
## DeeDee Summary

TODO decide fate then adapt
...
#deedee_summary(deedee_list = dde_macrophage,
...
deedee_app()
```
To explore a dataset with the app, one can either call the function with the DDE as an argument, or upload the DDE as an RDS in the App's input tab. 

The App entails one tab for each of the above presented functions. The respective parameters can be set via self-explanatory input boxes in each tab. Additional interactive functionality, namely the [interactive heatmap](#deedee-heatmap), a brushing (area selection) of points in the Q-Q and scatter plot and (TODO decide ORA fate) the possibility to run an over-representation on the brushed points in the scatter plot are implemented.
(TODO add info panels back) More information on the usage can be found in the `INFO` panel in each respective tab.
