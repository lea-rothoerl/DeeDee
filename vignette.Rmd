...
The function `deedee_cat` creates a Concordance At the Top plot depicting the share of genes that are concordant in the upper x ranks of two or more logFC-sorted input datasets. The highest rank for which the concordance is calculated is given by the `maxrank` (default = 1000) parameter. 

```{r, echo = TRUE, warning = FALSE, message = FALSE}
deedee_cat(data = mydata, maxrank = 15000)
```
