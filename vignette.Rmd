...
source("~/Development/DeeDee_wip/testdata_macrophage.R", local = knitr::knit_global())
source("~/Development/DeeDee_wip/deedee_venn.R", local = knitr::knit_global())
source("~/Development/DeeDee_wip/deedee_upSet.R", local = knitr::knit_global())
source("~/Development/DeeDee_wip/deedee_heatmap.R", local = knitr::knit_global())
source("~/Development/DeeDee_wip/deedee_qq.R", local = knitr::knit_global())
source("~/Development/DeeDee_wip/deedee_cat.R", local = knitr::knit_global())
source("~/Development/DeeDee_wip/deedee_scatter.R", local = knitr::knit_global())
source("~/Development/DeeDee_wip/deedee_idr.R", local = knitr::knit_global())
```
...
mydata <- list(naive_IFNg = inp1, IFNg_both = inp2, naive_Salm = inp3, Salm_both = inp4)
```
...
deedee_venn(data = mydata, mode = "both")
```
...
The function `deedee_scatter` creates a scatterplot of logFC values of the genes in two input datasets. If the input data list contains more than two datasets (like our example `mydata` does),  the `select1` (default = 1) and `select2` (default = 2) parameters specify which ones are to be used. `deedee_scatter` includes a `color_by` parameter that can be set to "pval1" (default), "pval2", "pval_mean", or "idr". The points will be colored from blue to red (low to high values). For more information on the "idr" coloring option, see [`deedee_idr`](#deedee-idr).

```{r, echo = TRUE, warning = FALSE, message = FALSE}
deedee_scatter(data = mydata, select1 = 2, select2 = 3, color_by = "pval_mean")
```
...
The function `deedee_heatmap` creates a heatmap of the logFC values for all common genes in every input dataset. A color key with a histogram of the times a certain color was used are displayed in the top left corner. Additionally to the standard parameters explained in [Input](#what-to-provide), heatmap takes a numeric `show_first` value (default = 500), specifying the number of genes depicted (if the total number of genes is smaller than `show_first`, all genes are shown), and a logical `show_gene_names` value (default = FALSE) that determines if the gene identifiers (rownames in `deedee_prepare` results) will be displayed in the heatmap. 

```{r, echo = TRUE, warning = FALSE, message = FALSE}
deedee_heatmap(data = mydata)
```
...
The function `deedee_qq` compares the statistical distributions of two input datasets. If the input data list contains more than two datasets (like our example `mydata` does),  the `select1` (default = 1) and `select2` (default = 2) parameters specify which ones are to be used. If the resulting curve resembles a straight line with a slope of 1, the distributions are similar (perfect straight line = identical distributions). `deedee_qq` includes a `color_by` parameter that can be set to "logFC1", "logFC2", "pval1" (default), or "pval2". The points generating the curve will be colored from blue to red (low to high values). 

```{r, echo = TRUE, warning = FALSE, message = FALSE}
deedee_qq(data = mydata, select1 = 1, select2 = 3, color_by = "pval2")
```
...
The function `deedee_cat` creates a Concordance At the Top plot depicting the share of genes that are concordant in the upper x ranks of two or more logFC-sorted input datasets. If the input data list contains more than two datasets (like our example `mydata` does),  the `select1` (default = 1) and `select2` (default = 2) parameters specify which ones are to be used. The highest rank for which the concordance is calculated is given by the `maxrank` (default = 1000) parameter. 

```{r, echo = TRUE, warning = FALSE, message = FALSE}
deedee_cat(data = mydata, select1 = 2, select2 = 3, maxrank = 15000)
```
...
The function `deedee_idr` creates a table of [Irreproducible Discovery Rates](https://www.uio.no/english/services/it/research/sensitive-data/use-tsd/hpc/software/idr.html) for the intersection of genes in the input datasets. [`deedee_scatter`](#deede-scatter-plot) uses this calculation as a coloring option. 

```{r, echo = TRUE, warning = FALSE, message = FALSE, eval = FALSE}
deedee_idr(data = mydata)
```
